...
--- API compatibility helpers to handle renamed functions in newer OBS versions (32.x+)
...
elseif ffi.os == "OSX" then
    ...
    if ok and lib ~= nil then
        ...
        -- Load CoreGraphics for display height detection
    ...
    if socket_mouse ~= nil then
        ...
        elseif ffi.os == "OSX" then
            if osx_lib ~= nil and osx_nsevent ~= nil and osx_mouse_location ~= nil then
                ...
                -- Fallback to CoreGraphics if monitor_info is not available
    ...
    if crop_filter_info ~= nil and zoom_target ~= nil then
        ...
        if zoom_state == ZoomState.ZoomingOut or zoom_state == ZoomState.ZoomingIn then
            ...
            if is_following_mouse then
                ...
                if not skip_frame and (zoom_target.crop.x ~= crop_filter_info.x or zoom_target.crop.y ~= crop_filter_info.y) then
